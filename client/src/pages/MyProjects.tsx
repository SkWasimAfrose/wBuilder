    ...
    <div className='px=4 md:px-16 lg:px-24 xl:px-32'>
      {loading ? (
            ...
            {projects.map((project) => (
              <div onClick={() => navigate(`/projects/${project.id}`)} key={project.id} className='relative group w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden shadow-md group hover:shadow-indigo-700/30 hover:border-indigo-800/80 transition-all duration-300'>
                ...
                <div className='p-4 text-white bg-linear-180 from-transparent group:hover:from-indigo-950 to-transparent transition-colors'>
                  ...
                  <p className='text-sm text-gray-400 mt-1 line-clamp-2'>{project.initialPrompt}</p>
                  ...
                  <div onClick={(e) =>e.stopPropagation()} className='flex justify-between items-center mt-6'>
                    ...
                    <div className='flex gap-3 text-white text-sm'>
                      <button onClick={() => navigate(`/preview/${project.id}`)} className='px-3 py-1.5 bg-white/10 rounded hover:bg-white/15 rounded-md transition-all'>Priview</button>
                      <button onClick={() => navigate(`/projects/${project.id}`)} className='px-3 py-1.5 bg-white/10 rounded hover:bg-white/15 rounded-md transition-colors'>Open</button>
                    </div>
                  </div>
                </div>
                ...
                <div onClick={(e) => e.stopPropagation()} className='absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity'>
                  <Trash2Icon onClick={() => deleteProject(project.id)} className='bg-white p-1.5 size-7 rounded text-red-500 cursor-pointer hover:bg-red-50 transition-colors'/>
                </div>
        ...
        <div className='flex flex-col items-center justify-center h-[80vh]'>
          ...
          <button onClick={() => navigate('/')} className='text-white px-5 py-2 rounded-md bg-indigo-500 hover:bg-indigo-600 active:scale-95 transition-all'>
            Create New
          </button>
        </div>
      )}
