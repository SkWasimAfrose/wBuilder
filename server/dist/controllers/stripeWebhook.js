...
const stripe = new Stripe(process.env.STRIPE_SECRET_KEY); // [4]
export const stripeWebhook = async (req, res) => {
    const signature = req.headers['stripe-signature']; // [5]
    // Get Webhook Secret from Environment Variables
    const endpointSecret = process.env.STRIPE_WEBHOOK_SECRET; // [4], [5]
    let event;
    try {
        // Construct the event using the raw body, signature, and secret
        event = stripe.webhooks.constructEvent(req.body, signature, endpointSecret); // [5]
    }
    catch (error) {
    ...
    switch (event.type) {
        case 'payment_intent.succeeded': { // [6]
            const paymentIntent = event.data.object; // [6]
            ...
            const { transactionId, appId } = session.metadata; // [7]
